...
<body>
	...
	<form class="input-group mt-2 mb-3" method="post" action="" style="width: 400px; margin: 0 auto;">
		<input type="text" id="id" class="form-control input-group-lg " name="content" placeholder="请输入图书类别名称进行查询"/>
	...
	<table class="table table-hover table-bordered table-striped container-fluid" >
		...
		<tbody class="" align="center">
			...
			if(!empty($_POST['content'])){
				...
				$sql = "select * from book_type where type_name like '%$content%';";
			}
			else{
			...
			if($number>0){
				...
				for($i=0;$i<$endl;$i++){
					...
					if($i>=$p){
						...
						<a href='del_booktype.php?id=$row[0]'>删除</a>
						<a href='up_booktype.php?id=$row[0]'>修改</a>
						</td></tr>";
					}
				}
